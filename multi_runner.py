...
def run_simulation(spray_method, direct_method, flows):
    ...
    max_lengths, max_sum_lengths = sim.simulate(
        flows,
...
labels = ['random/random', 'choice/random', 'choice/choice']
colors = ['blue', 'orange', 'green']
plt.figure(figsize=(10, 6))
for data, label, color in zip(results_sorted, labels, colors):
    plt.hist(data,alpha=0.5, label=label, color=color)

plt.title("Histogram of Max Queue Lengths for Different Routing Strategies")
plt.xlabel("Max Queue Length")
plt.ylabel("Nodes")
plt.legend()
plt.savefig(f'Overlapping Histograms')
